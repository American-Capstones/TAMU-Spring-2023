...
export const useGetSeverityCountsForOrg = async () => {
    ...
    let TeamsInOrg = await useGetTeamsForOrg("baggage-claim-incorporated", 10);

    for (var Team of TeamsInOrg) {
        let ReposForTeams = await useGetRepositoriesForTeam("baggage-claim-incorporated", Team.name, 10);

        for (var Repo of ReposForTeams) {
            if (!ReposVisited.includes(Repo.ID)){
                let vulns = await getVulnerabilitiesFromRepo(Repo.name, "baggage-claim-incorporated");
