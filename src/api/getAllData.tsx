...
import { Org, Team, Topic, Repository, vulnData} from "../utils/types";
    ...
    for(let vulns of newVulnsFormatted){
      ...
      if(!seen.has(id)){
        for (let topic of newRepo.repositoryTopics){
          ...
          seenTopics.get(topic)!.vulnData.moderate = seenTopics.get(topic)!.vulnData.moderate.map((a, i) => a + topicVulnData.moderate[i]);
          seenTopics.get(topic)!.vulnData.low = seenTopics.get(topic)!.vulnData.low.map((a, i) => a + topicVulnData.low[i]);
